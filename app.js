...
let schemaMetadataPromise = null;
let schemaMetadataCache = null;
function toolGroupId(toolId) {
    if (toolId.startsWith("beam_"))
        return "beam";
    if (toolId.startsWith("ec1_"))
        return "ec1";
    if (toolId.startsWith("ec2_"))
        return "ec2";
    if (toolId.startsWith("ec3_"))
        return "ec3";
    if (toolId.startsWith("ec5_"))
        return "ec5";
    if (toolId.startsWith("ec6_"))
        return "ec6";
    return "composite";
}
function groupLabel(groupId, lang) {
    return TOOL_GROUP_TEXT[groupId].label[lang] || TOOL_GROUP_TEXT[groupId].label.en;
}
function activeText(toolId, lang) {
    const tool = TOOL_CATALOG[toolId];
    return tool?.title?.[lang] || tool?.title?.en || toolId;
}
function toolSearchText(toolId) {
    const tool = TOOL_CATALOG[toolId];
...
export function buildFilteredToolGroups(query = "", lang = "en") {
...
function renderToolTabs(toolId, query, lang) {
    ...
    return `<div class="tool-nav-groups">${groups.map((group) => (`<section class="tool-nav-group">
      <div class="tool-nav-group-title">${escapeHtml(group.label)}</div>
      ${group.items.map((item) => (`<button type="button" data-tool-id="${item.toolId}" ${item.toolId === toolId ? 'aria-current="true"' : ""}>${escapeHtml(item.title)}</button>`)).join("")}
    </section>`)).join("")}</div>`;
}
export function buildToolContextModel(toolId, lang = "en") {
    const groupId = toolGroupId(toolId);
    const group = TOOL_GROUP_TEXT[groupId];
    const override = TOOL_CONTEXT_OVERRIDES[toolId];
    const assets = (TOOL_REFERENCE_ASSETS[toolId] || []).map((asset) => ({
        src: asset.src,
        alt: asset.alt,
...
function renderToolContext(container, toolId, lang) {
    ...
    container.innerHTML = [
        ...
        `</div>`,
        context.assets.length ? `<div class="tool-asset-gallery">${context.assets.map((asset) => (`<figure class="tool-asset-card">
...
function buildContactSummaryLines(response, lang) {
...
function loadSchemaMetadata() {
    if (schemaMetadataCache)
        return Promise.resolve(schemaMetadataCache);
...
function rootFieldName(field) {
    const path = Array.isArray(field.path) && field.path.length ? field.path : String(field.name || "").split(".");
    return String(path[0] ?? "");
}
export function buildFriendlyFieldHelp(toolId, tool, field, lang = "en", schemaMetadata = schemaMetadataCache) {
    const override = FRIENDLY_FIELD_HELP_OVERRIDES?.[toolId]?.[field.name];
...
export function buildVisualizationHtml(response, lang) {
    if (!response?.calculator_id || !response?.result)
        return "";
    const cards = [];
...
function buildBeamVisualizationCard(response, lang) {
    ...
    if (toolId === "beam_simple_diagrams") {
        ...
        if (!Array.isArray(x) || !Array.isArray(shear) || !Array.isArray(moment))
            return "";
    ...
    if (toolId === "beam_continuous_strip" || toolId === "beam_spring_calibration" || toolId === "beam_support_fixity_bracketing") {
        const supportPositions = response.result.support_positions_m
            || response.result.fixed_case?.reactions?.map((_, index) => index * ((numberOrNull(response.result.length_m) || 1) / Math.max((response.result.fixed_case?.reactions?.length || 2) - 1, 1)));
        const sample = response.result.sampled_response || response.result.average_response?.diagrams;
        if (!sample?.x_m || !sample?.moment_knm)
            return "";
...
function buildEc2VisualizationCard(response, lang) {
    if (response.calculator_id !== "ec2_rectangular_section_capacity")
        return "";
    ...
    const bars = layers.flatMap((layer) => distributeLayerBars(layer.n_bars, layer.depth_mm, width, layer.phi_mm));
...
function buildEc3VisualizationCard(response, lang) {
    const toolId = response.calculator_id;
    ...
    if (toolId === "ec3_bolted_lap_joint") {
        const svg = buildBoltGridSvg(numberOrNull(response.input?.plate_width_mm) || 140, 180, numberOrNull(response.input?.num_cols) || 2, numberOrNull(response.input?.num_rows) || 2);
        const metrics = buildMetricBars([
            { label: "Utilization", value: utilizationPercent(response.result.utilization_ratio), display: formatVisualizationRatio(response.result.utilization_ratio) },
    ...
    if (toolId === "ec3_bolted_moment_connection" || toolId === "ec3_splice_moment_connection" || toolId === "ec3_double_sided_web_connection") {
        const svg = buildConnectionRowsSvg(Array.isArray(response.input?.bolts?.rows) ? response.input.bolts.rows : [], numberOrNull(response.input?.end_plate?.width_mm) || 220, numberOrNull(response.input?.end_plate?.height_mm) || numberOrNull(response.input?.primary?.height_mm) || 360, numberOrNull(response.input?.bolts?.horizontal_spacing_mm) || 100);
        const momentResistance = response.result.mj_rd_knm
            ?? response.result.moment_resistance_knm
            ?? response.result.moment_resistance_knm
            ?? null;
        const utilization = response.result.overall_utilization
...
function buildEc5VisualizationCard(response, lang) {
    ...
    if (response.calculator_id === "ec5_steel_timber_screw_connection"
        ...
        const applied = response.input?.f_d_kn
            ?? response.input?.f_v_ed_kn
            ?? null;
        const resistance = response.result.r_d_kn
    ...
    if (response.calculator_id === "ec5_timber_contact_moment_joint") {
        const svg = buildJointSectionSvg(numberOrNull(response.input?.interface?.width_mm) || 160, numberOrNull(response.input?.interface?.height_mm) || 360, Array.isArray(response.input?.screws) ? response.input.screws : []);
        const metrics = buildMetricBars([
            { label: "Utilization", value: utilizationPercent(response.result.utilization_ratio), display: formatVisualizationRatio(response.result.utilization_ratio) },
...
function buildEc6VisualizationCard(response, lang) {
    if (response.calculator_id === "ec6_masonry_contact_pressure") {
        const svg = buildMasonryBearingSvg(numberOrNull(response.result.beam_width_mm) || 1, numberOrNull(response.result.support_length_mm) || 1, numberOrNull(response.result.required_length_mm) || 1);
        const metrics = buildMetricBars([
    ...
    if (response.calculator_id === "ec6_beam_bearing") {
        const svg = buildMasonryBearingSvg(numberOrNull(response.input?.bearing_width_mm) || 1, numberOrNull(response.input?.bearing_length_mm) || 1, numberOrNull(response.result.effective_length_mm) || 1);
        const metrics = buildMetricBars([
...
function buildLineChartSvg(xValues, yValues, title, unit) {
    if (!Array.isArray(xValues) || !Array.isArray(yValues) || !xValues.length || xValues.length !== yValues.length)
        return "";
    const width = 280;
    const height = 120;
    const padding = 16;
...
function buildBeamSketchSvg(span, input, supportPositions) {
    const width = 280;
    const height = 90;
    const padding = 20;
    const supports = supportPositions?.length
        ...
        : [0, span];
    const loads = [
        ...(Array.isArray(input?.point_loads) ? input.point_loads.map((load) => ({ position: Number(load.position_m), type: "point" })) : []),
        ...(Array.isArray(input?.distributed_loads) ? input.distributed_loads.map((load) => ({ position: Number(load.start_m), type: "distributed" })) : []),
...
function buildRectSectionSvg(widthMm, heightMm, fill = "#d6a85a") {
    const width = 220;
    const height = 220;
    const pad = 24;
...
function buildResidualSectionSvg(widthMm, heightMm, residualWidthMm, residualHeightMm) {
    const width = 220;
    const height = 220;
    const pad = 24;
    const scale = Math.min((width - 2 * pad) / widthMm, (height - 2 * pad) / heightMm);
...
function distributeLayerBars(count, depthMm, widthMm, phiMm) {
    if (!count || count <= 0)
        return [];
    if (count === 1) {
...
function buildEc2SectionSvg(widthMm, heightMm, bars) {
    const width = 220;
    const height = 220;
    const pad = 24;
    const scale = Math.min((width - 2 * pad) / widthMm, (height - 2 * pad) / heightMm);
...
function buildEc2StrainProfileSvg(strainProfiles, lang) {
    const profileSpecs = [
    ...
    const traces = profileSpecs.flatMap((spec) => {
        ...
        if (parsed.some((point) => point.epsilon === null || point.height === null))
            return [];
        return [{
                ...spec,
                points: parsed,
    ...
    const mapX = (epsilon) => padLeft + ((epsilon - xMin) / xSpan) * (width - padLeft - padRight);
    const mapY = (sectionY) => padTop + (sectionY / ySpan) * (height - padTop - padBottom);
    const zeroX = mapX(0);
    const axisBottomY = height - padBottom;
    const legendX = width - padRight - 74;
    const title = (lang === "nl" ? "Rekprofiel ε (‰)"
    ...
    <svg class="visual-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(title)}">
      ...
      ${traces.flatMap((trace) => trace.points.map((point) => (`<circle cx="${roundVisual(mapX(point.epsilon), 2)}" cy="${roundVisual(mapY(point.height), 2)}" r="3.2" fill="${trace.color}"></circle>`))).join("")}
...
function buildBoltGroupSvg(bolts, criticalBoltId) {
    const width = 220;
    const height = 220;
    const pad = 26;
...
function buildBoltGridSvg(widthMm, heightMm, cols, rows) {
    const bolts = [];
...
function buildConnectionRowsSvg(rows, plateWidthMm, plateHeightMm, horizontalSpacingMm) {
    const width = 220;
    const height = 240;
    const pad = 24;
...
function buildWeldStripSvg(lengthMm, throatMm) {
    const width = 240;
    const height = 120;
    const pad = 20;
...
function buildISectionSvg(section) {
...
function buildPlateSectionSvg(widthMm, thicknessMm) {
    const width = 220;
    const height = 180;
    const pad = 24;
...
function buildFastenerArraySvg(count, widthMm, heightMm, kind = "bolt") {
...
function buildJointSectionSvg(widthMm, heightMm, screws) {
    const width = 220;
    const height = 240;
    const pad = 22;
...
function buildMasonryBearingSvg(widthMm, supportLengthMm, requiredLengthMm) {
    const width = 240;
    const height = 140;
    const pad = 18;
...
function buildWallPanelSvg(widthMm, heightMm, compressedWidthMm) {
    const width = 240;
    const height = 180;
    const pad = 18;
    const safeWidth = Math.max(widthMm, 1);
...
function ratioPercent(actual, limit) {
    const numerator = numberOrNull(actual);
    const denominator = numberOrNull(limit);
    if (numerator === null || denominator === null || Math.abs(denominator) < 1e-9)
        return 0;
    return (numerator / denominator) * 100;
}
function inverseLimitPercent(actual, limit) {
    const numerator = numberOrNull(actual);
    const denominator = numberOrNull(limit);
    if (numerator === null || denominator === null || Math.abs(numerator) < 1e-9)
        return 0;
...
function formatVisualizationRatio(value) {
    const numeric = numberOrNull(value);
...
function roundVisual(value, decimals = 2) {
...
export function buildReportHtml(response, lang = "en", generatedAt = new Date()) {
    ...
    if (!report)
        return "";
    const visuals = buildVisualizationHtml(response, lang);
    return [
        "<!DOCTYPE html>",
...
export function initApp(documentRef = globalThis.document) {
    ...
    const overview = documentRef.querySelector("[data-tool-overview]");
    const tabs = documentRef.querySelector("[data-tool-tabs]");
    const tabsMeta = documentRef.querySelector("[data-tool-nav-meta]");
    const searchLabel = documentRef.querySelector("[data-tool-search-label]");
    ...
    const visual = documentRef.querySelector("[data-result-visual]");
    const contact = documentRef.querySelector("[data-result-contact]");
    const downloadButton = documentRef.querySelector("[data-download]");
    const downloadHtmlButton = documentRef.querySelector("[data-download-html]");
    ...
    async function handleContactSubmit(event) {
        ...
        const fallbackLink = contact.querySelector("[data-contact-fallback]");
        if (status)
            status.textContent = TEXT[state.lang].contactStatusIdle;
        const payload = {
            email: contact.querySelector("[data-contact-email]")?.value?.trim() || "",
            project_name: contact.querySelector("[data-contact-project]")?.value?.trim() || "",
            message: contact.querySelector("[data-contact-message]")?.value?.trim() || "",
            lead_tracking_consent: contact.querySelector("[data-contact-consent]")?.checked || false,
        ...
        if (fallbackLink)
            fallbackLink.href = directMailto;
        try {
            ...
            if (status)
                status.textContent = TEXT[state.lang].contactStatusSubmitted;
    ...
    function activateTurnstile() {
        ...
        loadTurnstileScript(documentRef)
            .then((turnstile) => {
            if (!turnstile || target.dataset.loaded === "true")
                return;
    ...
    function renderContactPanel(response) {
        ...
        const formEl = contact.querySelector("[data-contact-form]");
        ...
        contact.querySelectorAll("[data-contact-project], [data-contact-email], [data-contact-message], [data-contact-consent]").forEach((field) => {
            field.addEventListener("input", () => {
                state.contact = {
                    projectName: contact.querySelector("[data-contact-project]")?.value || "",
                    email: contact.querySelector("[data-contact-email]")?.value || "",
                    message: contact.querySelector("[data-contact-message]")?.value || "",
                    consent: contact.querySelector("[data-contact-consent]")?.checked || false,
                };
                persistContactState(state.contact);
            });
    ...
    downloadHtmlButton.addEventListener("click", () => {
        if (!lastResponse)
            return;
        const toolId = lastResponse.calculator_id || state.toolId;
        triggerDownload(documentRef, buildReportFilename(toolId), buildReportHtml(lastResponse, state.lang), "text/html");
    });
...
function renderFriendlyForm(container, toolId, tool, lang, heading) {
    ...
    container.innerHTML = [
        ...
        `<div class="friendly-fields">${tool.form.fields.map((field) => renderFriendlyField(toolId, tool, field, valueAtPath(tool.sample, field.path || [field.name]), lang)).join("")}</div>`,
    ].join("");
}
